...
class EfficientAdditiveAttnetion(nn.Module):
    def __init__(self, in_dims=512, token_dim=256, num_heads=2):    
        '''
            in_dims: 输入维度（输入的token的维度）
            token_dim: (token维度)(注意力机制的内部维度)，头数乘以token维度等于线性层的输出维度，注意输出维度也是这么多
            num_heads: 头数
        '''
        super().__init__()
...
if __name__ == '__main__':
    block = EfficientAdditiveAttnetion(64, 64).cuda()  # 创建模型实例并移动到CUDA设备

    input = torch.rand(3, 64 * 64*2, 64).cuda()  # 创建随机输入张量并移动到CUDA设备
